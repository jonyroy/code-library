//Interval Tee using Augmenting Red Black Binary Search Tree
#include<bits/stdc++.h>
using namespace std;
#define max_s 1432005
int p[max_s],Left[max_s],Right[max_s],key[max_s][2],color[max_s];
int Red=1,Black=0,nil=0,root=0,Max[max_s];

int Interval_Search(int low,int high)//find any interval are overlaps
{
    int x=root;
    while(x!=nil&&!(low<=key[x][1]&&high>=key[x][0]))
    {
        if(Left[x]!=nil&&Max[Left[x]]>=low)
        x=Left[x];
        else
        x=Right[x];
    }
...
int main(){
    ...
    while(1)
    {
        ...
        if(Interval_Search(low,high))
        cout<<"Interval are overlaps"<<endl;
        else
        cout<<"Interval are not overlaps"<<endl;
    }
